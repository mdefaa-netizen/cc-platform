    ...
    if sel:
        ...
        with st.form("edit_nhh_form"):
            ...
            if save:
                ...
                st.rerun()
            if delb:
                st.session_state["_confirm_delete_nhh"] = sel

        if st.session_state.get("_confirm_delete_nhh") == sel:
            st.warning("Are you sure you want to delete this colleague?")
            c_yes, c_no = st.columns(2)
            with c_yes:
                if st.button("Yes, delete", key="confirm_del_nhh"):
                    delete_nhh(sel)
                    st.session_state.pop("_confirm_delete_nhh", None)
                    st.success("Deleted.")
                    st.rerun()
            with c_no:
                if st.button("Cancel", key="cancel_del_nhh"):
                    st.session_state.pop("_confirm_delete_nhh", None)
                    st.rerun()
