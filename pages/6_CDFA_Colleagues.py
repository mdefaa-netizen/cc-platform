...
with tab_add:
    ...
    with st.form("add_cdfa_form", clear_on_submit=True):
        c1, c2 = st.columns(2)
        with c1:
        ...
        if st.form_submit_button("💾 Save Colleague", use_container_width=True):
            if not name:
                ...
                st.session_state["cdfa_just_added"] = {"name": name}
                st.balloons()
                st.rerun()

    # Post-save confirmation banner — shown after a successful add
    if "cdfa_just_added" in st.session_state:
        info = st.session_state["cdfa_just_added"]
        st.success(f"✅ Colleague added: \"{info['name']}\".")
        st.info("👉 Click the **All CDFA Colleagues** tab above to see them in the list.")
        if st.button("➕ Add Another Colleague", use_container_width=True, key="add_another_cdfa"):
            del st.session_state["cdfa_just_added"]
            st.rerun()
